...
已登记数字 ID 的手工成片可用 `clip_upload_registry.py rebuild --id ... --start ... --end ...`
调整窗口和文案，再用 `cut --ids ... --review-note ...` 按原编号重压。
旧手工元数据首次重建时，必须提供 `--review-note` 和 `--source-kind`；程序先核对源字幕在原窗口内
与已有成片字幕的文字、时间一致，再绑定来源指纹。后续来源变动仍会拦截，原视频和字幕保留在旧路径。
这些命令不授权投稿，详见 `docs/topic-event-editorial.md` 的已成片重压流程。

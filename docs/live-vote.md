...
- 上述三选项会播报 `投票30秒，发序号：1.复联2 2.老头环 3.美队3`；中途例如 `剩余20秒~1.复联2:0票 2.老头环:1票 3.美队3:0票`；结束例如 `结束：1.复联2:0票 2.老头环:1票 3.美队3:0票 【老头环】胜~`。胜出项用 `【】` 突出显示，超长时保持胜出项完整并另发一条。平票和无人投票会明确标注。
- 中途票型在完整消息能放进单条字数上限时，加上 `剩余N秒~`。剩余秒数按生成票型时距截止时间计算并向上取整；若加上倒计时会超长，则保留 `票型：` 前缀并沿用原有拆分，优先保留选项名称和票数，不为了倒计时增加弹幕条数。跨房间发送排队仍可能带来延迟。

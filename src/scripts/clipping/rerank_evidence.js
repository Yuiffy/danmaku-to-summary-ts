...
const CANDIDATE_COLUMNS = ['q', 's', 'l', 'm', 'a', 'r', 'e', 'v', 'g', 'd', 'top'];
...
function buildRerankEvidence(candidates, parsed, danmaku, config) {
    ...
    const records = candidates.map(candidate => {
        ...
        if (reusableRecall(candidate, subtitleEvidence, config, danmaku)) {
    ...
    const recallHints = candidates.map(candidate => ({ index: candidate.index, value: reusableRecall(candidate, subtitleEvidence, config, danmaku) }))
        .filter(item => item.value);
    const reuseLines = recallHints.length ? [
        '=== 可复用的召回定位 ===',
        '这些索引、时间范围和引用已由程序校验，内容真伪仍以全文为准。没有改变选材内容时可直接沿用，避免重复换算。',
        ...recallHints.map(item => `#${item.index} ${JSON.stringify(item.value)}`)
    ] : [];
    ...
    return {
        ...
        danmakuLines: [
            'D行格式为ID、显示秒数、JSON文本或Tref对象；精确时间由ID映射保留。T指向同文文本字典；发言引用只用D-ID，不用R或T。',
            ...audienceRows.map(row => `${row.id} ${Math.floor(row.time)} ${JSON.stringify(

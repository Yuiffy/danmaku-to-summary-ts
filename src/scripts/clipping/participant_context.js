...
function buildParticipantContext(rootConfig, info, parsed, danmaku, metadata = {}) {
    ...
    const people = buildPersonEvidenceContext(rootConfig, info?.roomId).map(person => {
        ...
        return { ...person, preferredName: preferredName(entry) || person.label,
...
function identityDanmaku(danmaku, window, context, limit = 4) {
    ...
    const people = context.people.map(person => ({ person, matches: nameMatcher(person.names) }));
    const selected = [];

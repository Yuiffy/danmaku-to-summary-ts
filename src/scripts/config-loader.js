...
const AISchema = Joi.object({
    ...
    text: Joi.object({
        ...
        daiYu: Joi.object({
            ...
            model: Joi.string().default('gpt-6-luna'),
            fallbackModels: Joi.array().items(Joi.string()).default(['gemini-3-flash-preview']),
            thinking: Joi.object({
                enabled: Joi.boolean().default(true),
...
const ConfigSchema = Joi.object({
    ...
    clipTopics: Joi.object({
        ...
        aiModel: Joi.string().default('gpt-6-luna'),
    ...
    ownStreamClips: Joi.object({
        ...
        maxCandidates: Joi.number().default(80),
        maxClips: Joi.number().default(50),
        publicationPolicy: Joi.object({
            mode: Joi.string().valid('all', 'score', 'curated', 'shadow'),
            roomIds: Joi.array().items(Joi.alternatives(Joi.string(), Joi.number())).min(1),
            minScore: Joi.number().min(0).max(100),
            maxStandalone: Joi.number().integer().min(1).max(100),
            standoutScore: Joi.number().min(0).max(100),
            bundles: Joi.object({ enabled: Joi.boolean(), minScore: Joi.number().min(0).max(100),
                protectScore: Joi.number().min(0).max(100), maxGroups: Joi.number().integer().min(1).max(5),
                maxMembers: Joi.number().integer().min(2).max(4), maxSeconds: Joi.number().min(30).max(900),
                minCombinedScore: Joi.number().min(0).max(100), minScoreGain: Joi.number().min(0).max(100),
                maxCandidates: Joi.number().integer().min(2).max(50) }).optional()
        }).optional(),
        chunkSeconds: Joi.number().default(2700),
        aiConcurrency: Joi.number().default(3),
        streamReviewRendering: Joi.boolean().default(false),
        ...
        ai: Joi.object({
            ...
            retry: TextRetrySchema.optional(),
            retryByStage: Joi.object().pattern(Joi.string().valid('recall', 'rerank', 'detail', 'actorReview', 'dialogueEvidence'), TextRetrySchema).optional(),
            rankThenEdit: Joi.object({ enabled: Joi.boolean(), cardChars: Joi.number().integer().min(300).max(2000),
                rankMaxTokens: Joi.number().integer().min(1000).max(50000), detailBatchSize: Joi.number().integer().min(1).max(6),
                detailConcurrency: Joi.number().integer().min(1).max(8), detailMaxTokens: Joi.number().integer().min(1000).max(50000),
                detailTimeoutMs: Joi.number().integer().min(1000).max(1200000) }).optional(),

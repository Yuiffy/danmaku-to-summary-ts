...
async function generateClipTitle(context = {}) {
    ...
    const prompt = [
        ...
        '3. 必须点出一个具体看点：反差、翻车、嘴硬、弹幕急眼/破防/炸锅、锐评、离谱发言、操作失误、当场社死、越说越怪等。',
        '4. 优先使用主播原话、弹幕反应、游戏/事件名，形成“具体事件 + 反应/槽点”的结构。',
        '5. 准确描述切片中主播说了什么/做了什么，不要编造。',
        '6. 要结合上下文理解字幕——ASR可能有同音错字，要根据语境推断正确意思。',
        '7. 如果提到特定主播名字，直接用名字（岁己/小岁/栞栞/米米等），不要加引号。',
        '8. 不要用“直播有趣片段”“精彩瞬间”“很可爱的一段”“聊到了XX”这种弱标题。',
        '',
        '人工标题参考风格：',

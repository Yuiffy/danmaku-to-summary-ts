...
test('serializes the requested strict schema in each protocol and omits it for ordinary text', () => {
  ...
  const options={model:'gpt-6-luna',prompt:'Return the summary.',maxTokens:2000,responseFormat:format};
...
test('selected cache routing preserves the existing role and full source without unsupported Responses fields', () => {
  ...
  const selected = getExplicitPromptCachePlan(prompt, config, 'gpt-6-luna', 100);
  const options = { model: 'gpt-6-luna', prompt, maxTokens: 100000, thinkingEnabled: true, reasoningEffort: 'high' };
...
test('preserves the uncached request role and payload after the unsuccessful role experiment', () => {
  const options = { model: 'gpt-6-luna', prompt: 'An ordinary clip task.', maxTokens: 100000,
...
test('implicit routing preserves the original input layout and task role outside explicit rollout', () => {
    const prompt = `${context.SHARED_PROMPT_CACHE_START}\nExact facts.\n${context.SHARED_PROMPT_CACHE_END}\nTask.`;
    const cachePlan = getExplicitPromptCachePlan(prompt, {}, 'gpt-6-luna', 0);
    expect(cachePlan.enabled).toBe(false);
    expect(cachePlan).not.toHaveProperty('prefix');
    const body = buildDaiYuResponsesRequest({ model: 'gpt-6-luna', prompt, maxTokens: 1000, cachePlan });
    ...
    const chat = buildDaiYuChatCompletionsRequest({ model: 'gpt-6-luna', prompt, maxTokens: 1000, cachePlan });
...
test.each(['disabled', 'unsupported', 'no-prefix'])('does not add a route or split when %s', mode => {
  ...
  const model = mode === 'unsupported' ? 'unsupported-model' : 'gpt-6-luna';
...
test('different task suffixes keep the same implicit prefix and route without adding a system role', () => {
  ...
  const bodies = ['Goodnight task.', 'Comic task.'].map(task => {
    const prompt = prefix + '\n\n' + task;
    const cachePlan = getExplicitPromptCachePlan(prompt, {}, 'gpt-6-luna', 0);
    return buildDaiYuResponsesRequest({ model: 'gpt-6-luna', prompt, cachePlan, maxTokens: 100000,
...
test.each(['gpt-6-luna', 'gpt-6-sol', 'gpt-6-astra'])('forwards max without silently downgrading %s', model => {

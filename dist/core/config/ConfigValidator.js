...
class ConfigValidator {
    ...
    static validate(config) {
        ...
        if (!config.app) {
            ...
            if (!config.app.name)
                errors.push({ path: 'app.name', message: 'App name is required', type: 'required' });
            if (!config.app.version)
                errors.push({ path: 'app.version', message: 'App version is required', type: 'required' });
            if (!config.app.environment)
                errors.push({ path: 'app.environment', message: 'App environment is required', type: 'required' });
            if (!config.app.logLevel)
                errors.push({ path: 'app.logLevel', message: 'App log level is required', type: 'required' });
        }
        if (!config.webhook) {
            errors.push({ path: 'webhook', message: 'Webhook configuration is required', type: 'required' });
        }
        else {
            if (config.webhook.port === undefined)
                errors.push({ path: 'webhook.port', message: 'Webhook port is required', type: 'required' });
            if (!config.webhook.host)
                errors.push({ path: 'webhook.host', message: 'Webhook host is required', type: 'required' });
        }
        if (!config.audio) {

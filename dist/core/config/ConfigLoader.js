...
class ConfigLoader {
    ...
    async load(options = {}) {
        ...
        let config = {};
        // 1. 加载默认配置
        const defaultConfig = ConfigValidator_1.ConfigValidator.getDefaultConfig();
        ...
        // 6. 验证配置
        if (validate) {
            const validationResult = ConfigValidator_1.ConfigValidator.validate(config);
     ...
     * 保存配置到文件
     */
    async save(config, targetPath) {
        const savePath = targetPath || this.configPath;
        const currentConfig = this.config || ConfigValidator_1.ConfigValidator.getDefaultConfig();
        const mergedConfig = this.deepMerge(currentConfig, config);
        // 验证配置
        const validationResult = ConfigValidator_1.ConfigValidator.validate(mergedConfig);
        if (!validationResult.valid) {

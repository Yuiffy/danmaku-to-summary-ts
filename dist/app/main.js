...
class MainApplication {
    ...
    async start(options) {
        try {
            ...
            this.logger = (0, LogManager_1.getLogger)('MainApplication');
     ...
     * 处理单个文件（CLI模式）
     */
    async processFile(videoPath, xmlPath, roomId) {
        try {
 ...
 * 应用命令行参数到配置
 */
function applyCommandLineOptions(options) {
    try {
        const config = ConfigProvider_1.ConfigProvider.getConfig();
        if (options.port !== undefined) {
            config.webhook.port = options.port;
 ...
 * 在配置初始化后应用命令行参数
 */
function applyCommandLineOptionsAfterInit(options) {
    try {
        const config = ConfigProvider_1.ConfigProvider.getConfig();
        if (options.port !== undefined) {
            config.webhook.port = options.port;

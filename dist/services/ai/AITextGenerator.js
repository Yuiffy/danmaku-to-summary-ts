...
exports.AITextGenerator = void 0;
exports.createAITextGenerator = createAITextGenerator;
const fs = require("fs");
...
class AITextGenerator {
    logger = (0, LogManager_1.getLogger)('AITextGenerator');
    config;
    provider;
    providerConfig = null;
    constructor() {
    ...
    loadConfig() {
        try {
            return ConfigProvider_1.ConfigProvider.getConfig();
        }
        catch (error) {
    ...
    getNames(roomId) {
        ...
        const roomConfig = this.config.ai?.roomSettings?.[roomId];
        if (!roomConfig) {
            return defaultNames;
        }
        return {
     ...
     * 构建晚安回复提示词
     */
    buildGoodnightPrompt(highlightContent, roomId) {
...
请根据以上直播内容，以${fan}的身份写一篇晚安回复。记住：只使用提供的直播内容，不要添加任何外部信息。`;
     ...
     * 读取高亮文件内容
     */
    readHighlightFile(highlightPath) {
        try {
            return fs.readFileSync(highlightPath, 'utf8');
        }
        catch (error) {
            throw new AppError_1.AppError(`读取AI_HIGHLIGHT文件失败: ${error instanceof Error ? error.message : error}`, 'FILE_READ_ERROR', 500);
        }
    }
    /**
     * 保存生成的文本
     */
    saveGeneratedText(outputPath, text, highlightPath) {
        try {
...
`;
            ...
            throw new AppError_1.AppError(`保存生成文本失败: ${error instanceof Error ? error.message : error}`, 'FILE_WRITE_ERROR', 500);
        }
    }
    /**
     * 使用Gemini生成文本
    ...
    async generateWithGemini(prompt, options) {
        if (!this.providerConfig?.apiKey) {
            throw new AppError_1.AppError('Gemini API密钥未配置', 'CONFIGURATION_ERROR', 400);
        }
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new AppError_1.AppError('Gemini API调用超时', 'TIMEOUT_ERROR', 408)), timeout);
            });
            ...
            throw new AppError_1.AppError(`Gemini API调用失败: ${error instanceof Error ? error.message : error}`, 'AI_SERVICE_ERROR', 500);
        }
    }
    /**
     * 生成文本
    ...
    async generateText(prompt, options) {
        if (!this.isConfigured()) {
            throw new AppError_1.AppError('AI服务未配置', 'CONFIGURATION_ERROR', 400);
        }
        switch (this.provider) {
            case 'gemini':
                return await this.generateWithGemini(prompt, options);
            case 'openai':
                throw new AppError_1.AppError('OpenAI提供者暂未实现', 'NOT_IMPLEMENTED_ERROR', 501);
            default:
                throw new AppError_1.AppError(`不支持的AI提供者: ${this.provider}`, 'CONFIGURATION_ERROR', 400);
        }
    }
    /**
     * 生成晚安回复
     */
    async generateGoodnightReply(highlightPath, roomId) {
        ...
        try {
            ...
            if (!fs.existsSync(highlightPath)) {
                throw new AppError_1.AppError(`AI_HIGHLIGHT文件不存在: ${highlightPath}`, 'FILE_NOT_FOUND_ERROR', 404);
            }
            // 读取内容
     ...
     * 批量生成晚安回复
     */
    async batchGenerateGoodnightReplies(directory) {
        const results = [];
        try {
     ...
     * 构建自定义提示词
     */
    buildCustomPrompt(content, options) {
...
exports.AITextGenerator = AITextGenerator;
/**
 * 创建AI文本生成服务实例
 */
function createAITextGenerator() {
    return new AITextGenerator();
}
//# sourceMappingURL=AITextGenerator.js.map